...
for (let index = 0; index < files.length; index += 4) {
  await Promise.all(files.slice(index, index + 4).map(async file => {
    ...
    assert.ok(live.includes('/contact-ui.css?v=20260916-intent1'), route);
    if (local.includes('class="fab-container"')) {
      ...
      assert.ok(live.includes('class="contact-invitation"'), route);
      ...
      assert.ok(live.includes('/contact-ui.js?v=20260916-intent1'), route);
    }
  }));
}
...
assert.equal(missingPage.status, 404, 'Unknown paths must return a real 404');
assert.match(await missingPage.text(), /<meta name="robots" content="noindex, follow">/, '404 page must stay out of the index');
