...
for (let index = 0; index < files.length; index += 4) {
  await Promise.all(files.slice(index, index + 4).map(async file => {
    ...
    assert.ok(live.includes('/contact-ui.css?v=20260904-balloons3'), route);
    if (local.includes('class="fab-container"')) {
      ...
      assert.equal((live.match(/class="contact-tip"/g) || []).length, 4, route);
      assert.ok(live.includes('/contact-ui.js?v=20260904-balloons3'), route);
    }
  }));
}
