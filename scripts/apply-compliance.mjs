...
const legal = {
    pl: {
        ...
        manage: 'Ustawienia prywatności', updated: 'Ostatnia aktualizacja: 6 września 2026 r.', diagnostics: '<h2>Narzędzie pogodowe i diagnostyka łącza</h2><p>Strona diagnostyczna wyświetla użytkownikowi jego publiczny adres IP, przybliżoną lokalizację sieci, operatora i techniczne parametry udostępnione podczas obsługi żądania przez Cloudflare. Odpowiedzi diagnostyczne nie są buforowane, a adres IP i wyniki nie są zapisywane ani wysyłane do analityki. Dane pogodowe MET Norway pobieramy po stronie serwera, dlatego MET Norway nie otrzymuje adresu IP odwiedzającego z przeglądarki.</p>',
        privacy: `
    ...
    ru: {
        home: '/ru/', homeLabel: 'Вернуться на NaSerwis.pl', privacyTitle: 'Политика конфиденциальности', cookieTitle: 'Политика cookies', manage: 'Настройки конфиденциальности', updated: 'Последнее обновление: 6 сентября 2026 г.', diagnostics: '<h2>Погода и диагностика соединения</h2><p>Страница показывает вам публичный IP, примерное сетевое местоположение, оператора и параметры Cloudflare. Ответ не кэшируется; IP и результаты не сохраняются и не отправляются в аналитику. Прогноз MET Norway получает наш сервер, поэтому браузер не передаёт сервису ваш IP.</p>',
    ...
    uk: {
        home: '/uk/', homeLabel: 'Повернутися на NaSerwis.pl', privacyTitle: 'Політика конфіденційності', cookieTitle: 'Політика cookies', manage: 'Налаштування конфіденційності', updated: 'Останнє оновлення: 6 вересня 2026 р.', diagnostics: '<h2>Погода й діагностика з’єднання</h2><p>Сторінка показує вам публічну IP-адресу, приблизне мережеве розташування, оператора й параметри Cloudflare. Відповідь не кешується; IP і результати не зберігаються та не надсилаються до аналітики. Прогноз MET Norway отримує наш сервер, тому браузер не передає сервісу вашу IP-адресу.</p>',
    ...
    en: {
        home: '/en/', homeLabel: 'Return to NaSerwis.pl', privacyTitle: 'Privacy policy', cookieTitle: 'Cookie policy', manage: 'Privacy settings', updated: 'Last updated: 6 September 2026', diagnostics: '<h2>Weather and connection diagnostic</h2><p>The page shows you the public IP address, approximate network location, provider and parameters available to Cloudflare. The response is not cached; the IP and measurements are neither stored nor sent to analytics. Our server fetches the MET Norway forecast, so the visitor browser does not send its IP to that service.</p>',
...
function legalPage(language, type) {
    ...
    const body = ((type === 'privacy' ? t.privacy + t.diagnostics : t.cookies))
